...
#         self.back.pack_propagate(0) #Don't allow the widgets inside to determine the frame's width / height
        ...
        self.scCFpos = {'row': 4, 'column': 0, 'columnspan': 2} # used in _stchange as well
        self.scCondFrame.grid(**self.scCFpos)
        ...
        self.tcCFpos = {'row': 1, 'column': 0, 'columnspan': 2} # this frame will have to be remade elsewhere
        self.tcCondFrame.grid(**self.tcCFpos)
        
        self.GPFpos = {'row': 1, 'column': 0, 'columnspan': 2, 'rowspan': 2} # shared b/w A, B
        self.FLFpos = {'row': 2, 'column': 2, 'columnspan': 2}
        self.RPFpos = {'row': 2, 'column': 2, 'columnspan': 2}
        ...
        self.tccw['A']['FLFrame'].grid(**self.FLFpos) 
        ...
        self.tccw['C']['RPFrame'].grid(**self.RPFpos)
...
#         self.ttoolbar.update()
    ...
    def _change_p1(self, *args):
        ...
        if p1T == 'animation':
            ...
            tk.Label(master=self.p1_conditional_controls, text="filename").grid(row=0, column=0)
            pathSaveFnameE = tk.Entry(master=self.p1_conditional_controls)
            pathSaveFnameE.grid(row=0, column=1)
            pathSaveFnameE.insert(0, "path.txt")
            ...
            if self.prevP1Type != 'puller potential energy':
                ...
                tk.Label(master=self.p1_conditional_controls, 
                         text="puller potential energy: ").grid(row=1, column=0)
            self.fig1.clear()
            self.ax1 = self.fig1.add_subplot(111)
            if hasattr(self, "tp"):
                ...
                if (tmin != "") and (tmax != ""):
                    ...
                    ppe = CDTtools.dyn_core_tools.puller_potential_energy(self.tp, theta0, plot=True, 
                                                                          axPPE=self.ax1)
                tk.Label(master=self.p1_conditional_controls, text=str(ppe)).grid(row=1, column=1)
                self.canvas1.draw()
            ...
            tk.Label(master=self.p1_conditional_controls, text="filename").grid(row=0, column=0)
            default_dir = os.path.join(toplevel_path, "saved_designs")
            arlSaveFnameE = tk.Entry(master=self.p1_conditional_controls)
            arlSaveFnameE.grid(row=0, column=1)
            arlSaveFnameE.insert(0, "axle_load_data.txt")
    ...
    def _check_solveproc(self):
        ...
        if not (self.solve_proc).is_alive():
            ...
            self._check_solveproc() # next call will find the process dead and return
        else:
    ...
    def _calcfit(self):
        ...
        tf_msg = tf_msg[:-3]
        F0_msg = '\nPuller force at initial configuration: ' + str(round(self.F0, nc))
        if self.Fmin < 0:
    ...
    def _wrapup(self):
        ...
        if self.cpr.poll(0.1):
            ...
            sim_res_msg = (extra_msg + "Launch speed: " + str(round(vpf, self.rp)) + 
            ...
            sim_res_msg = ("Solver did not finish. This may be caused by excessively short time steps, or the " 
                           ...
                           "helpful to read the logfile or increase the timeout")
